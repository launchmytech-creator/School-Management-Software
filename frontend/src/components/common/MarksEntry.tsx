...
const MarksEntry: React.FC<MarksEntryProps> = ({ layout }) => {
  ...
  const handleSaveMarks = async () => {
    ...
    const marksData = studentMarks
      ...
      .map(sm => ({
        ...
        marksObtained: sm.isAbsent ? undefined : parseFloat(sm.marksObtained) || 0,
        grade: sm.isAbsent ? undefined : calculateGrade(parseFloat(sm.marksObtained) || 0, selectedSubjectData.maxMarks),
        isAbsent: sm.isAbsent,
      }));
              ...
              <table className="w-full">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-center text-xs font-bold text-slate-500 uppercase tracking-wider">Absent</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {studentMarks.map((student) => {
                    ...
                    const grade = student.isAbsent ? 'AB' : calculateGrade(marks, maxMarks);
                    ...
                    return (
                      <tr key={student.studentId} className="hover:bg-slate-50/50">
                        ...
                        <td className="px-6 py-4 text-center">
                          <button
                            onClick={() => handleAbsentToggle(student.studentId)}
      ...
      >
        <div className="p-6 space-y-4">
          <p className="text-slate-600">
            Are you sure you want to save marks for <span className="font-semibold">{studentMarks.filter(s => s.marksObtained || s.isAbsent).length}</span> students?
          </p>
  ...
  if (isAdmin) {
    return <AdminLayout title="Marks Entry"><div className="space-y-6 pb-12">{renderContent()}</div></AdminLayout>;
  }

  return <AccountantLayout title="Marks Entry"><div className="space-y-6 pb-12">{renderContent()}</div></AccountantLayout>;
};
