...
import { Search, Send, Phone, AlertTriangle } from 'lucide-react';
import { useAcademicYear } from '../../context/AcademicYearContext';
import { useClasses } from '../../hooks/queries/useClasses';
import { useFeeDefaulters } from '../../hooks/queries/useFeeTransactions';
...
interface FeeClassDefaultersProps {
  layout: 'admin' | 'accountant';
  canSendReminders: boolean;
}

const FeeClassDefaulters: React.FC<FeeClassDefaultersProps> = ({
  layout,
  canSendReminders,
}) => {
  ...
  const withContactCount = defaulters.filter((d) => d.parentPhone || d.parentEmail).length;
  ...
  const sendReminder = async () => {
    ...
    try {
      ...
      if (!parentId) {
        showNotification(
          'No parent account linked to this student. Please contact administrator to link parent account.',
          'error',
        );
        return;
      }
      ...
      showNotification(
        'Failed to send reminder. Please try again or contact administrator.',
        'error',
      );
    } finally {
  ...
  if (!classData) {
    return (
      <EmptyState
        ...
        action={{ label: 'Go Back', onClick: () => navigate(`${basePath}/fee-defaulters`) }}
      />
    );
  }
  ...
  return (
    <div className="space-y-6 pb-12">
      <PageHeader
        title={`Fee Defaulters - ${classData.name}`}
        subtitle={
          classData.section
        ...
        breadcrumb={{
          links: [
            { label: 'Finance', href: `${basePath}/fee-defaulters` },
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        ...
                        {defaulter.parentPhone && (
                          <a
                            href={`tel:${defaulter.parentPhone}`}
                      ...
                      <td className="px-6 py-4 text-center">
                        {canSendReminders && (
                          <Button
                            ...
                            disabled={
                              !defaulter.parentEmail && !defaulter.parentPhone
                            }
                          >
...
export default FeeClassDefaulters;
