...
const ParentDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAnnouncements = async () => {
      try {
        const data = await announcementService.getAnnouncements({ limit: 4 });
  ...
  const presentDays =
    selectedChild?.recent_attendance.filter((a) => a.status === "present")
      .length ?? 0;
  ...
  return (
    <div className="p-8 space-y-6 max-w-7xl mx-auto">
      <div className="relative">
        <div
          ref={tabsRef}
          className="flex items-center gap-1 border-b border-slate-200 overflow-x-auto scrollbar-hide px-10"
        >
          {children.map((child) => (
            <button
      ...
      {selectedChild && (
        <>
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-4 mt-1 flex-wrap">
                <span className="flex items-center gap-1 text-xs text-slate-500">
                  <span
                    className="material-symbols-outlined text-[14px] text-[#4A9FD4]"
                    style={{ fontVariationSettings: "'FILL' 1" }}
                  >
                    school
                  </span>
                  {selectedChild.student.class_name || "N/A"}
                  {selectedChild.student.class_section
                    ? ` - Section ${selectedChild.student.class_section}`
                    : ""}
                </span>
                <span className="flex items-center gap-1 text-xs text-slate-500">
                  <span
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            >
              <p
                className={`text-2xl font-black ${hasPendingFees ? "text-rose-600" : "text-slate-900"}`}
              >
