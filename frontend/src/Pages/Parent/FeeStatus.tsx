...
const FeeBreakdownBadges: React.FC<{ breakdown: Record<string, number> }> = ({ breakdown }) => (
  <div className="flex flex-wrap gap-1 mt-1">
    {Object.entries(breakdown).map(([type, amount]) => (
      <span key={type} className="text-[10px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded font-medium">
        {type}: {fmt(amount)}
      </span>
    ))}
  </div>
    ...
    el.innerHTML = `
      ...
      <table>
        ...
        <tbody><tr>
          <td>
            ${txLabel(tx)}
            ${tx.feeBreakdown ? `<div class="breakdown">${Object.entries(tx.feeBreakdown).map(([k,v]) => `${k}: ${fmt(v)}`).join(' | ')}</div>` : ''}
          </td>
                    ...
                    {transactions.map((tx, i) => (
                      ...
                      >
                        <td className="px-6 py-4">
                          ...
                          {tx.feeBreakdown && <FeeBreakdownBadges breakdown={tx.feeBreakdown} />}
                        </td>
